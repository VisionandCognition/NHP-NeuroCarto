...
import numpy as np
from bokeh.models import UIElement, ColumnDataSource, GlyphRenderer, Slider, Switch, Div, CheckboxGroup, tools, CustomJS
...
class BoundView(ViewBase, InvisibleView, metaclass=abc.ABCMeta):
    ...
    freeze_drag_checkbox: CheckboxGroup  # checkbox to freeze/unfreeze drag
    ...
    def __init__(self, config: CartoConfig, *,
        ...
        self._last_valid_boundary_data = None  # Track last valid state to prevent new box creation
        
        # Create freeze drag checkbox (will be added to title row in _setup_title)
        self.freeze_drag_checkbox = CheckboxGroup(labels=['Freeze Image Drag'], active=[], inline=True)
    ...
    def setup_boundary(self, f: Figure, *,
        ...
        self.data_boundary.selected.on_change('indices', as_callback(self._on_boundary_selection_change))

        # Custom drag handler using pan events - replaces BoxEditTool
        # This eliminates the unwanted box drawing behavior completely
        
        # Track press position for delayed drag activation
        f.js_on_event('press', CustomJS(args=dict(source=self.data_boundary, fig=f, freeze_checkbox=self.freeze_drag_checkbox), code="""
            // Check if drag is frozen
            if (freeze_checkbox.active.includes(0)) {
                return; // Drag is frozen, don't process
            }
            
            // Track press position for delayed drag activation
            
            // Initialize drag state on first use
            if (typeof window._boundary_drag === 'undefined') {
                window._boundary_drag = {
                    is_dragging: false,
                    start_x: 0,
                    start_y: 0,
                    rect_start_x: 0,
            ...
            // Check if press is inside the rectangle
            const rect_x = data['x'][0];
            const rect_y = data['y'][0];
            const rect_w = data['w'][0];
            const rect_h = data['h'][0];
            const half_w = rect_w / 2;
            const half_h = rect_h / 2;
            
            // Check if click is inside the rectangle
            const inside = (
                cb_obj.x >= rect_x - half_w &&
                cb_obj.x <= rect_x + half_w &&
                cb_obj.y >= rect_y - half_h &&
                cb_obj.y <= rect_y + half_h
            );
            
            // Remember press position and whether it was inside
            window._boundary_drag.last_press_x = cb_obj.x;
            window._boundary_drag.last_press_y = cb_obj.y;
            window._boundary_drag.press_inside = inside;
        """))
        
        f.js_on_event('panstart', CustomJS(args=dict(source=self.data_boundary, fig=f, freeze_checkbox=self.freeze_drag_checkbox), code="""
            // Check if drag is frozen
            if (freeze_checkbox.active.includes(0)) {
                return; // Drag is frozen, don't process
            }
            
            // Activate drag for immediate click-and-drag gestures
            ...
            if (inside) {
                ...
                window._boundary_drag.is_dragging = true;
                window._boundary_drag.start_x = cb_obj.x;
                window._boundary_drag.start_y = cb_obj.y;
                window._boundary_drag.rect_start_x = rect_x;
                window._boundary_drag.rect_start_y = rect_y;
            }
        """))
        
        f.js_on_event('pan', CustomJS(args=dict(source=self.data_boundary), code="""

            const data = source.data;
            if (data['x'].length === 0) return;
            
            // If not already dragging, check if we should start (for click-and-wait-then-move)
            if (!window._boundary_drag || !window._boundary_drag.is_dragging) {
                ...
                // Check if the last press was inside the rectangle
                if (window._boundary_drag.press_inside) {
                    const rect_x = data['x'][0];
                    const rect_y = data['y'][0];
                    console.log('[CustomDrag] Drag started (delayed from press)');
                    window._boundary_drag.is_dragging = true;
                    // Use the press position as the start, not current position
                    window._boundary_drag.start_x = window._boundary_drag.last_press_x;
                    window._boundary_drag.start_y = window._boundary_drag.last_press_y;
                    window._boundary_drag.rect_start_x = rect_x;
                    window._boundary_drag.rect_start_y = rect_y;
                } else {
                    return; // Last press wasn't inside, don't drag
                }
            }
            
            // Calculate new position based on delta from start
            const delta_x = cb_obj.x - window._boundary_drag.start_x;
            const delta_y = cb_obj.y - window._boundary_drag.start_y;
            
            const new_x = window._boundary_drag.rect_start_x + delta_x;
            const new_y = window._boundary_drag.rect_start_y + delta_y;
            

            
            // Create new data object to trigger update
            const new_data = {};
            for (let key in data) {
                if (key === 'x') {
                    new_data[key] = [new_x];
                    ...
                    new_data[key] = data[key].slice();
                }
            }
            source.data = new_data;
        """))
        
        f.js_on_event('panend', CustomJS(args=dict(source=self.data_boundary), code="""
            if (window._boundary_drag) {

                window._boundary_drag.is_dragging = false;
                // Don't clear press_inside - keep it for potential next drag
            }
        """))
        
        # Also handle mousemove for click-and-wait-then-move behavior
        # Pan events don't fire if you wait too long after clicking
        f.js_on_event('mousemove', CustomJS(args=dict(source=self.data_boundary, fig=f, freeze_checkbox=self.freeze_drag_checkbox), code="""
            // Check if drag is frozen
            if (freeze_checkbox.active.includes(0)) {
                return; // Drag is frozen, don't process
            }
            
            // Handle delayed drag (click-and-wait-then-move)
            
            // Only process if we have a recent press inside
            if (!window._boundary_drag || !window._boundary_drag.press_inside) return;
            
            const data = source.data;
            if (data['x'].length === 0) return;
            
            // If not dragging yet, check if we should start
            if (!window._boundary_drag.is_dragging) {
                // Check if mouse has moved significantly from press position (indicates drag intent)
                const dx = Math.abs(cb_obj.x - window._boundary_drag.last_press_x);
                const dy = Math.abs(cb_obj.y - window._boundary_drag.last_press_y);
                
                if (dx > 5 || dy > 5) { // Threshold to avoid jitter

                    window._boundary_drag.is_dragging = true;
                    window._boundary_drag.start_x = window._boundary_drag.last_press_x;
                    window._boundary_drag.start_y = window._boundary_drag.last_press_y;
                    window._boundary_drag.rect_start_x = data['x'][0];
                    window._boundary_drag.rect_start_y = data['y'][0];
                }
            }
        ...
        # Handle mouse release to end drag started by mousemove
        f.js_on_event('pressup', CustomJS(args=dict(source=self.data_boundary), code="""
            if (window._boundary_drag) {

                window._boundary_drag.is_dragging = false;
                window._boundary_drag.press_inside = false; // Clear to stop mousemove tracking
            }
        """))
    ...
    def setup_scale_slider(self, *,
        ...
        # Return as list containing a column: slider row, then checkbox on its own row
        from bokeh.layouts import column, row
        return [column(
            row(reset_ims, self.boundary_scale_slider),
            self.freeze_drag_checkbox
        )]
    ...
    def _on_boundary_selection_change(self, indices: list[int]):
        """Maintain selection to prevent area drawing behavior."""
        if len(indices) == 0 and len(self.data_boundary.data.get('x', [])) > 0:
            self.data_boundary.selected.indices = [0]

    @recursive_call_barrier
    def _on_boundary_change(self, value: dict[str, list[float]]):
        self.logger.debug(f'_on_boundary_change called with: x={value.get("x", [])}, y={value.get("y", [])}')
        # Prevent new box creation - ensure we only ever have one box
        if len(value.get('x', [])) > 1:
            # Multiple boxes detected, revert to last valid state
            self.logger.debug('Prevented new box creation, reverting to previous state')
            if self._last_valid_boundary_data is not None:
                self.data_boundary.data = self._last_valid_boundary_data
            return
        ...
        self._last_valid_boundary_data = dict(value)
