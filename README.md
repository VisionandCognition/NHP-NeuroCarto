...
**NHP-NeuroCarto** is an adaptation of the NeuroCarto package to increase compatibility with long NHP Neuropixels probes. It was hacked together by Chris Klink at the Netherlands Institute for Neuroscience ([c.klink@nin.knaw.nl](mailto:c.klink@nin.knaw.nl)).  The main difference in approach is that for NHPs we cannot rely on standard atlases, but we need an individualized planning. In our lab at the [Netherlands Institute for Neuroscience](https://nin.nl), we use [3D Slicer](https://www.slicer.org/) for much of our offline trajectory planning. NHP-NeuroCarto is configured to take a screenshot of an in-plane ('probe-view') and align it to an NHP-Neuropixels probe model for typical NeuroCarto channel selection. An extensive how-to is available in [How to use NHP-NeuroCarto](HowToNHPNeuroCarto.md).
